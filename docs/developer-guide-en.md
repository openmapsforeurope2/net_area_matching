...
This documentation, intended for developers, aims at detailing the harmonization process for "network area features" along borders, as well as the technical principles and configuration of the tool. "Network area features" refer to objects which are directly linked to the hydrographic network because they may contain fictitious watercourse_link axes. In the ECRM data models, these are standing_water and watercourse_area objects.
...
Installing the application requires compiling internal and external libraries, including those from IGN France.
...
The data on which this process is launched must first be "cleaned" using the **clean** tool from the [data-tools](https://github.com/openmapsforeurope2/data-tools) project, which removes features located outside of the country extent and too far from the international boundary.  
This tool should be used on working tables containing extracted data from both countries in the area around their common border.

## General Principle of the Process

The network area feature harmonization process is broken down into a series of key steps.
...
Each step is assigned a three-digit number and steps follow a strict order. If a step transforms the data it works on, the result is stored in a new table whose name is prefixed with the step number.  
...
**310** - generate 'cutting lines'  
**320** - remove areas outside their country  
**330** - clean up orphan 'cutting lines'  
**334** - create intersection areas between the two countries, stored in a dedicated table  
**335** - generate 'cutting points'  
**340** - merge overlapping areas between the two countries  
**350** - split merged areas using 'cutting lines' and sections from 'cutting points'  
**360** - assign attributes to the merged/split areas  
**370** - aggregate merged/split areas  
**399** - export 'standing water' areas
...
The following files may be found in the [configuration folder](https://github.com/openmapsforeurope2/net_area_matching/tree/main/config):
...
- db_conf.ini: database connection information. On the final infrastructure, this file is not required.
...
Objective: generate _'cutting lines'_, i.e., portions of contours shared by two polygons of the same country. Such lines mark separations between polygons of the same country, so probable attribute changes.
...
|-----------------|-------|--------|----------------|------------------------------|
| AREA_TABLE_INIT | X     | X      | X              | areas to process             |
| CUTL_TABLE      |       | X      |                | 'cutting lines' table        |
...
#### Process description:
Processing is done country by country. The principle is to build a planar graph from all polygon contours of a country, then identify arcs shared by two faces.

<img width="775" height="600" alt="image" src="https://github.com/user-attachments/assets/a01fd3e2-fa6a-4879-80aa-bee13b62deac" />

### 320: CleanByLandmask

Objective: remove areas and area parts too far away from their country.
...
|-----------------------|-------|--------|----------------|-------------------------------|
| AREA_TABLE_INIT       | X     | X      | X              | areas to process              |
| TARGET_BOUNDARY_TABLE | X     |        |                | international borders         |
...
Areas extending beyond their country limit are cut along the international boundary, offset by a certain distance outside the country.
...
##### 2) Remov areas outside the country
...
| PC_COUNTRY_BUFFER      | radius of the work buffer used to simplify the national extent, needed to determine if a polygon is inside its country |

Areas located outside their country and further than a certain threshold from the boundary are removed.
...
This step removes 'cutting lines' which, following the cleaning in the previous step, do not have at least one adjacent area belonging to the same country.
...
| parameter                | description                                                                                        |
|--------------------------|----------------------------------------------------------------------------------------------------|
| LINKED_FEATURE_ID        | Field containing the national identifiers of the source polygons of the overlapping area              |
| NATIONAL_IDENTIFIER_NAME | Field name for the national identifier                                                             |

The operator iterates through all the areas in the _AREA_TABLE_INIT_ table belonging to 'country 1' and calculates the overlap areas with each area in this table belonging to 'country 2'. Each overlap area is stored in the _INTERSECTION_AREA_TABLE_.
...
The calculation of _'cutting points'_ consists of iterating through the areas in _AREA_TABLE_INIT_ for each of the two neighbouring countries. For each area, it is determined if a splitting point should be created at the end of a median axis.
...
This step deals with merging the areas of the two neighboring countries.
...
After merging the areas of the two neighbouring countries, and then splitting these merged areas by the _'cutting features'_, this step assigns attributes to these new areas.
...
If area A represents less than 10% of area B, the merged area takes B's attributes. If B represents less than 10% of A, the merged area takes A's attributes.
...
This step aims at aggregating as much as possible the multitude of areas generated by the merge/split process.
...
| SAM_SMALL_AREA_LENGTH_THRESHOLD | Length threshold for a small area (length of the medial axis)               |
| NATIONAL_IDENTIFIER_NAME        | Field name for the national identifier                                      |
| W_TAG_NAME                      | Working field here used to mark areas resulting from merging                |

All processing described below applies only to areas resulting from merging polygons from the two neighboring countries (objects with a non-null _W_TAG_NAME_ field).

The first step is to establish a list of small areas ordered by their size (ordering ensures repeatability of aggregations). Only areas meeting the small area criterion are added.

Secondly, an attempt is made to form groups of objects to be merged. For this, small areas are processed in descending order of size, and for each, its largest neighboring area with the same _W_TAG_NAME_ value is sought.
...
In this step, areas from the _'watercourse areas'_ working table marked as _'standing waters'_ are exported and re-imported into the _'standing waters'_ working table.
...
- **Cutting line**: edge representing the section of contour shared between two adjacent polygons of the same country. This edge is used for area splitting.
- **Cutting point**: Specific point on the contour of a polygon located at the end of the medial axis. This point is used as a reference to create a splitting section.
